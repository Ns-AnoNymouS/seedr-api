...
module.exports = class Seedr {
  ...
  async getFilesById(id = null) {
    ...
    var res = {parentId: parent, name: data.data.name, folderSize: 0, totalStorage: data.data.space_max, usedStorage: data.data.space_used, type: data.data.type, activeTorrents: data.data.torrents, files: []};
    for (var folder of data.data.folders) {
        res.files.push({
            id: folder.id,
            type: 'folder',
